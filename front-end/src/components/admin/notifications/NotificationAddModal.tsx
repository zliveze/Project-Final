...
interface NotificationAddModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<Notification>) => void;
}

const NotificationAddModal: React.FC<NotificationAddModalProps> = ({
  isOpen,
  onClose,
  onSubmit
}) => {
  ...
  const handleSubmit = async (data: Partial<Notification>) => {
    try {
      ...
      toast.error('Có lỗi xảy ra khi thêm thông báo. Vui lòng thử lại sau.');
    } finally {
