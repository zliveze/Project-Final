...
export default async function handler(
  ...
  if (!token && (req.method === 'PATCH' || req.method === 'DELETE')) {
  ...
  try {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'https://backendyumin.vercel.app'}/reviews/${id}`;
    ...
    if (req.method === 'GET') {
      ...
      // Gọi API backend để cập nhật đánh giá
      const response = await fetch(apiUrl, {
        method: 'PATCH',
        headers,
